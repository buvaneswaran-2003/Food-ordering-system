...
const fetchCart = async() =>{
    await axios.get('http://localhost:3001/fetch-cart').then(
      (response)=>{
  ...
  const removeItem = async(id) =>{
    await axios.put('http://localhost:3001/remove-item', {id}).then(
      (response)=>{
        fetchCart();
        fetchCartCount();
      }
  ...
  const placeOrder = async() =>{
    if(cart.length > 0){
        await axios.post('http://localhost:3001/place-cart-order', {userId, name, mobile, email, address, pincode, paymentMethod, orderDate: new Date()}).then(
          (response)=>{
            alert('Order placed!!');
            setName('');
            setMobile('');
